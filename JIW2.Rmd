...
iTraxx_EUR <- rename(iTraxx_EUR, iTraxx_EUR = `Last Price`)
...
iTraxx_EXJP <- rename(iTraxx_EXJP, iTraxx_EXJP = `Last Price`)
...
iTraxx_AUS <- rename(iTraxx_AUS, iTraxx_AUS = `Last Price`)
...
iTraxx_JPN <- rename(iTraxx_JPN, iTraxx_JPN = `Last Price`)
...
CDX_NA <- rename(CDX_NA, CDX_NA = `Last Price`)
...
CDX_EM <- rename(CDX_EM, CDX_EM = `Last Price`)
...
percentages_merged$log_iTraxx_EUR <- log(percentages_merged$iTraxx_EUR)
percentages_merged$log_iTraxx_EXJP <- log(percentages_merged$iTraxx_EXJP)
percentages_merged$log_iTraxx_AUS <- log(percentages_merged$iTraxx_AUS)
percentages_merged$log_iTraxx_JPN <- log(percentages_merged$iTraxx_JPN)
percentages_merged$log_CDX_NA <- log(percentages_merged$CDX_NA)
percentages_merged$log_CDX_EM <- log(percentages_merged$CDX_EM)
...
ggplot(monthly, aes(x = factor(month), y = pdiff)) +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 1),  strip.text = element_text(size = 10, face = "bold")) +
